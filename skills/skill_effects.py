    ...
    async def _apply_skill_effect(self, skill_name: str, skill_data: Dict,
        ...
        try:
            ...
            # === 전역 효과 스킬 ===
                ...
                # 몬스터가 사용: 유저에게 -10 / 유저가 사용: 몬스터에게 -10
                is_caster_monster = caster_id in ["monster", "admin"]
                is_target_monster = user_id in ["monster", "admin"]
                
                if (is_caster_monster and not is_target_monster) or \
                   (not is_caster_monster and is_target_monster):
