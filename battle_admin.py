...
class AdminBattleManager:
    ...
    async def handle_dice_roll(self, user_id: int, channel_id: int, dice_value: int):
        ...
        if SKILL_SYSTEM_AVAILABLE:
            try:
                # ✅ 스킬 시스템을 위한 올바른 user_id 결정
                skill_user_id = str(user_id)  # 기본값: 실제 user_id를 문자열로
                if battle.admin and user_id == battle.admin.user.id:
                    skill_user_id = "admin"  # admin인 경우 "admin"으로 설정
                
                print(f"[DEBUG] Skill system - user_id: {skill_user_id}, original: {user_id}")
                
                # ✅ 올바른 user_id를 스킬 시스템에 전달
                final_value, skill_messages = await self._process_dice_with_skill_effects(
                    channel_id, skill_user_id, dice_value  # user_name 대신 skill_user_id 사용
                )
                
                print(f"[DEBUG] Skill effects applied - original: {dice_value}, final: {final_value}")
    ...
    async def _process_dice_with_skill_effects(self, channel_id: int, skill_user_id: str, dice_value: int) -> Tuple[int, List[str]]:
        """주사위 처리에 스킬 효과 적용"""
        if not SKILL_SYSTEM_AVAILABLE or not skill_effects:
            return dice_value, []
            
        try:
            print(f"[DEBUG] Calling skill_effects.process_dice_roll with user_id: {skill_user_id}")
            
            # 스킬 시스템의 주사위 처리 함수 호출
            final_value, skill_messages = await skill_effects.process_dice_roll(
                skill_user_id, dice_value, str(channel_id)  # ✅ 올바른 user_id 전달
            )
            
            print(f"[DEBUG] Skill effects result - original: {dice_value}, final: {final_value}")
    ...
    async def _apply_skill_damage_effects(self, channel_id: int, target_id: str, base_damage: int) -> int:
        ...
        try:
            # ✅ 현재는 데미지 스킬 효과가 구현되지 않았으므로 기본값 반환
    ...
    async def _trigger_skill_round_start_events(self, channel_id: str):
        ...
        try:
            from skills.skill_adapter import get_skill_handler
    ...
    async def handle_post_damage_skill_effects(self, channel_id: int, target_player, damage: int):
        ...
        try:
            ...
            # skill_effects의 데미지 공유 처리 메서드 사용
            shared_damage = await skill_effects.process_damage_sharing(
                channel_id=str(channel_id),
                victim_id=str(target_player.user.id),
                damage=damage
            )
            
            # 공유된 데미지 적용
            if shared_damage:
                for target_id, shared_dmg in shared_damage.items():
                    if target_id == "all_alive":
                        # 모든 살아있는 플레이어에게 데미지
                        for player in self.get_alive_players(channel_id):
                            if player.user.id != target_player.user.id:
                                await self.apply_damage(player, shared_dmg, "카론 데미지 공유")
                    else:
                        # 특정 대상에게 데미지
                        target = self.get_player_by_id(channel_id, target_id)
                        if target:
                            await self.apply_damage(target, shared_dmg, "스킬 데미지 공유")
    ...
    async def handle_recovery_with_hwangya(self, interaction: discord.Interaction, 횟수: int):
        ...
        try:
            ...
            # 황야 행동 카운터 업데이트 (skill_effects를 통해)
            hwangya_instance = skill_effects.get_skill_instance(channel_id, "황야")
            if hwangya_instance and hwangya_instance.active:
                # 황야 스킬의 행동 카운트 증가
                if hasattr(hwangya_instance, 'use_action'):
                    hwangya_instance.use_action(user_id, "recovery")
...
async def health_check() -> bool:
    """시스템 헬스 체크"""
    global cleanup_task  # ← global 선언을 함수 맨 처음으로 이동
