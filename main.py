...
async def setup_skill_system(bot):
    from skills.skill import setup
    await setup(bot)
    ...
    if message.author.bot:
        ...
        if message.author.id == 218010938807287808:  # 다이스 봇 ID
            ...
            if match:
                ...
                # process_dice_message 메서드를 직접 호출
...
async def handle_multi_battle_command(message: discord.Message):
    ...
    if " vs " in content:
        ...
        if len(parts) != 2:
            await message.channel.send("올바른 형식: !전투 @유저1 @유저2 vs @유저3 @유저4, 체력값들")
            return
        ...
        # 팀 B와 체력값 파싱
        team_b_and_health = parts[1].split(',', 1)
        team_b_part = team_b_and_health[0].strip()
        ...
        if not team_a_mentions or not team_b_mentions:
            await message.channel.send("각 팀에 최소 1명씩 멘션해주세요.")
            return
        
        # 체력값 파싱
        health_values = []
        if len(team_b_and_health) > 1:
            health_part = team_b_and_health[1].strip()
        ...
        # 체력값이 부족하면 10으로 채우기
        total_users = len(team_a_mentions) + len(team_b_mentions)
        while len(health_values) < total_users:
            health_values.append(10)
        
        # 팀 전투 시작
        await admin_manager.start_team_battle_with_sync_choice(
            message, team_a_mentions, team_b_mentions, health_values
        )
...
# main.py의 몹ai테스트 명령어를 다음과 같이 교체하세요:

# ===== 추가 명령어: AI 테스트 =====
@tree.command(name="몹ai테스트", description="몹 AI 및 전체 시스템을 종합적으로 테스트합니다 (Admin 전용)")
@app_commands.describe(
...
# 타격 명령어 수정
@tree.command(name="타격", description="⚔️ Admin 전용 - 특정 대상에게 집중공격을 합니다")
...
async def on_ready():
    ...
    try:
        # Admin 전용 명령어는 따로 동기화할 수도 있음
        # 하지만 @is_admin() 체크로 충분함
        synced = await tree.sync()
        logger.info(f"{len(synced)}개의 명령어가 동기화되었습니다")
...
@tree.command(name="회복", description="회복 아이템을 사용하여 체력을 회복합니다. (Admin은 주사위로 회복)")
...
async def 회복_command(interaction: discord.Interaction, 아이템: Optional[str] = None):
    ...
    try:
        ...
        if is_in_battle and is_user_turn:
            await auto_skip_turn_after_recovery(interaction.user.id, channel_id, battle_type, interaction.user.display_name, health_recovered)  # health_recovered 추가
...
async def check_and_validate_battle_turn(user_id: int, channel_id: int) -> dict:
    ...
    Returns:
        dict: {
            "in_battle": bool,
            "is_user_turn": bool, 
            "battle_type": str  # "normal" or "admin" or None
        }
    """
    result = {
        "in_battle": False,
        ...
        if battle_type == "mob":
            ...
            if hasattr(bot, 'mob_battles') and channel_id in bot.mob_battles:
                ...
                if battle.pending_action and battle.pending_action.get("type") == "player_turn":
                    player = battle.pending_action.get("player")
                    if player and player.user.id == user_id:
                        # MobSetting을 통한 회복 처리 (실제 회복량 전달)
                        from mob_setting import MobSetting, DiceResult
                        mob_setting = MobSetting(bot)
...
@tree.command(name="게임시작", description="미니게임을 시작합니다.")
...
# main.py의 내카드 명령어 수정 (약 595번째 줄)
@tree.command(name="내카드", description="현재 게임에서 내 카드를 확인합니다.")
...
# 조커 게임 명령어들
@tree.command(name="카드뽑기", description="다른 플레이어의 카드를 뽑습니다. (조커 게임)")
...
@tree.command(name="게임상태", description="현재 게임 상태를 확인합니다.")
...
@tree.command(name="순서변경", description="조커 게임의 턴 순서를 랜덤으로 변경합니다.")
