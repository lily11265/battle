        ...
        try:
            ...
            try:
                ...
                if "SkillCog" not in bot.cogs:
                    ...
                    logger.info("✅ 스킬 시스템 Cog 로드 완료")
                else:
                    logger.info("✅ 스킬 시스템 Cog 이미 로드됨")
            except Exception as e:
                logger.error(f"❌ 스킬 Cog 로드 실패: {e}")
                # 계속 진행
            
            logger.info("✅ 스킬 시스템 초기화 완료")
            
        except Exception as e:
            logger.error(f"❌ 스킬 시스템 초기화 실패: {e}")
            # 스킬 시스템 없이도 봇이 작동하도록 계속 진행
...
async def perform_skill_system_health_check():
    ...
    try:
        ...
        # 3. 스킬 모듈 로드 확인
        try:
            # SKILL_MODULE_MAP을 heroes 모듈에서 가져오기
            from skills.heroes import SKILL_MODULE_MAP
            loaded_skills = len(SKILL_MODULE_MAP)
            logger.info(f"로드된 스킬 개수: {loaded_skills}")
            ...
            # 백업: skill.py에서 가져오기 시도
            try:
                from skills.skill import SKILL_MODULE_MAP
                loaded_skills = len(SKILL_MODULE_MAP)
